...
description: 统一 SubLingo IINA 插件的版本变更、基于用户指定版本规格且只描述用户功能变化的中文 release notes、质量检查、native helper 构建验收、`.iinaplgz` 正式打包、归档审计和产物摘要。用于请求提升插件版本、根据规格撰写 `docs/releases/vX.Y.Z.md`、准备发布候选包、生成正式安装包、撰写 GitHub Release 更新说明、重跑发布流水线或核验发布产物时。
---
...
创建或更新 `docs/releases/vX.Y.Z.md`，其中 `X.Y.Z` 必须与目标版本完全一致。文件使用中文并面向最终用户，标题使用 `# SubLingo vX.Y.Z`；只保留有内容的“新功能”、“功能改进”、“问题修复”和“升级与兼容提醒”章节。每条只说明用户场景、可感知功能变化及必要的启用条件或限制，不描述技术变更、内部模块、实现过程或验证方式。没有用户可感知变化时，在标题下明确说明“本版本没有面向用户的行为变化”。
