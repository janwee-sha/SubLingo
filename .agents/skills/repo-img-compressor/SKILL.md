...
description: 将 SubLingo 仓库 docs/ 下受版本控制的静态 PNG、JPEG 或 WebP 图片以 WebP quality 90 压缩，保持显示尺寸与文件名主体，在扩展名变化时同步更新 README 与 docs 文档引用并移除确认无引用的旧格式文件。用于用户要求压缩、优化、减小、转 WebP 或批量处理 SubLingo 文档图片时；不处理插件运行时资源、忽略目录中的实机测试截图、SVG 或动画图片。
---
...
1. 在仓库根目录读取 `AGENTS.md`、`docs/engineering/constitution.md` 和初始 `git status --short`。把此任务作为轻量文档资源维护：保护无关改动，不自动暂存，不触碰 `docs/open_issues/` 或 `docs/providers/`。
