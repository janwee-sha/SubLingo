...
- GitHub Release 中文正文：触发 commit、包内版本、精确大小、SHA-256、八项门禁、完整归档清单、两个 native 可执行文件验收结果及 FFmpeg 对应源码信息。
...
`dist/native/` 必须只含 `sublingo-transport` 与 `sublingo-subtitle-extractor`。构建文件与包内文件必须分别包含 `arm64` 和 `x86_64`、最低 macOS 12、可执行权限、有效签名、相同 SHA-256，且只依赖系统动态库。

FFmpeg 静态构建材料不得进入 `.iinaplgz`。审计任务只把正式包、正式包校验、锁定源码和源码校验纳入可发布白名单；`release-audit.json` 与 `release-notes.md` 是发布过程证据，不作为 Release 附件上传。
