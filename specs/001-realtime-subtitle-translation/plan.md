...
SubLingo 是 IINA 1.4+ 插件。每个播放器 main entry 独立读取外部 SRT/ASS、调度有限翻译批次、维护会话缓存并管理第二字幕；global entry 共享 Profile 元数据、凭据访问和单次 provider 调用；受限 Swift helper 提供可取消 HTTP、响应头读取、明确代理路由和固定用途凭据存储。
