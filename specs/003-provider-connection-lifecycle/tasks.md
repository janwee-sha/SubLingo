...
**目标**：系统代理连接在 helper 生命周期内有界复用，每 host 最多 4 条，同时保持任务级取消、重定向和 `direct` 行为。

**独立测试**：运行 native 契约测试，证明超过 4 个同 host 请求会排队并复用共享会话；取消/重定向只影响目标 job，关闭释放资源，`direct` 仍走 libcurl。
...
- **阶段 6**：依赖三个故事的聚焦验证全部通过；T024 严格依赖 T021、T022、T023。
...
- 25 个任务均符合 checkbox、连续 ID、故事标签和明确文件路径格式。
