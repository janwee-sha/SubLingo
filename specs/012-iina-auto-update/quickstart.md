...
预期：合法 0.3.3 身份通过；缺失、仓库不匹配、非整数、版本不一致及归档漂移全部失败。
...
预期：八项全部通过，生成 `build/package/SubLingo-0.3.3.iinaplgz`。
...
3. 打开插件管理面板，确认 SubLingo 显示为正式安装项且可卸载。
