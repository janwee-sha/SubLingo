...
- [X] T001 [P] 在 `tests/contract/plugin-update-metadata.test.ts` 先覆盖 0.3.3 合法身份、稳定版本映射、仓库不匹配、字段缺失、非法整数和版本漂移（FR-001–FR-004、SC-001–SC-003）
...
**目标**：正式 0.3.3 包包含 IINA 可读取且与公开仓库一致的更新身份。

**独立测试**：生产模块接受 `{version: 0.3.3, ghRepo: janwee-sha/SubLingo, ghVersion: 3003}`，正式归档保留相同字段，其他身份失败。

- [X] T007 [US1] 在 `Info.json` 增加 `ghRepo` 与 `ghVersion` 并把清单版本更新为 0.3.3（依赖 T005）
- [X] T008 [US1] 在 `package.json`、`package-lock.json` 两处项目版本和 `scripts/pack.sh` 两处安全路径把项目版本精确更新为 0.3.3（依赖 T007）
...
## 阶段 5：用户故事 3——旧版本用户获得一次性升级指引（优先级：P3）

**目标**：旧版用户知道需要手动安装 0.3.3 一次，0.3.3 之后可使用 IINA 更新检查。

**独立测试**：人工审阅安装说明和 0.3.3 发布说明，能够从单个升级提醒确定受影响版本、一次性操作和升级后行为。

- [X] T016 [P] [US3] 在 `README.md` 与 `docs/readme/README.zh-CN.md` 更新 GitHub 安装和更新说明，明确 0.3.2 及更早版本的一次性手动升级边界（FR-007–FR-008、SC-005）
- [X] T017 [P] [US3] 在其余 `docs/readme/README.*.md` 本地化文档同步同一更新语义，不扩展功能范围（FR-007–FR-008）
- [X] T018 [US3] 在 `docs/releases/v0.3.3.md` 根据本规格撰写中文用户发布说明，并在 `specs/004-automatic-github-release/spec.md` 移除已过时的“不新增自动更新机制”边界、引用 012 当前职责（FR-008）

## 阶段 6：完整验证与宿主验收

- [X] T019 依照 `specs/012-iina-auto-update/quickstart.md` 严格运行八项发布门禁并生成 `build/package/SubLingo-0.3.3.iinaplgz`
- [X] T020 对最终归档执行版本、更新身份、清单白名单、SHA-256、双 native helper 架构、执行权限和签名审计，并记录自动化通过与 IINA GUI 未覆盖状态
- [x] T021 由用户在 IINA 1.4.4 对最终 0.3.3 包完成人工安装、无错误更新检查、字幕翻译冒烟和卸载验收，再记录实际结果
...
2. 完成 P1 的清单、版本、staging 与归档路径，独立证明 0.3.3 包可更新。
3. 完成 P2 的发布阻断，确保未来版本不会回归。
4. 完成 P3 的旧版升级披露与 0.3.3 发布说明。
