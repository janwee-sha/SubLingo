...
- 目标身份 `0.3.4`、`ghVersion: 3004`，锁定 FFmpeg 源位于 `native/.build/ffmpeg/downloads/ffmpeg-8.1.2.tar.xz`。
...
- 删除成功立即收敛，迟到列表和重复事件不恢复条目；失败或取消保留业务状态。不同区域反馈互不覆盖，同一区域旧、重复或未知结果不能覆盖较新请求。
...
预期：当前身份为 `0.3.4`、`janwee-sha/SubLingo`、`3004`，目标归档为 `build/package/SubLingo-0.3.4.iinaplgz`；任一版本、正文或路径漂移都会失败。
...
SUBLINGO_COMMIT=$(git rev-parse HEAD)
SUBLINGO_NOTES_SHA=$(shasum -a 256 docs/releases/v0.3.4.md | awk '{print $1}')
mkdir -p build/release
node scripts/audit-release.mjs \
  --artifact build/package/SubLingo-0.3.4.iinaplgz \
  --expected-version 0.3.4 \
  --expected-github-repository janwee-sha/SubLingo \
  --expected-github-version 3004 \
  --expected-commit "$SUBLINGO_COMMIT" \
  --build-helper dist/native/sublingo-transport \
  --build-extractor dist/native/sublingo-subtitle-extractor \
  ...
  --release-notes docs/releases/v0.3.4.md \
  --release-notes-sha256 "$SUBLINGO_NOTES_SHA" \
...
1. 确认不存在同名开发链接，安装最终 `SubLingo-0.3.4.iinaplgz`，记录包 SHA-256、macOS、架构与 IINA 版本。
...
6. 取消一次原生删除确认并制造一次删除失败；条目及原业务状态保留，结果显示在原 Profile 行操作区。成功时原条目变为同位置的只读成功结果槽。
7. 依次及交错执行 Translate、Save Languages、Profile Save/Update、Select、Test、Delete 和 Subtitle Retry；busy 与终态均在对应控件正下方，不同区域互不覆盖，同一区域迟到结果不覆盖较新结果，屏幕阅读器不重复播报 Profile 行状态。
