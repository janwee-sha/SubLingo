...
**决策**：发布接口先读取全部 Release 状态。公开同版本直接跳过；无 Release 时创建 draft；匹配版本和提交的 draft 可继续。已有资产先下载并比较 SHA-256，完全一致才复用，缺失才上传，冲突立即失败。全部资产核对后才把 draft 公开并标记 Latest。
...
**决策**：workflow 顶层和构建任务使用 `contents: read`，只有依赖构建成功的发布任务使用 `contents: write`。发布正文由最终审计生成，校验文件和 Actions 日志共同作为证据，不回写仓库。

**理由**：GitHub Actions 可按 job 收窄 `GITHUB_TOKEN` 权限；不回写可避免递归 `main` 推送。Release 正文明确 CI 未覆盖 IINA GUI 安装、卸载和播放，但不阻塞自动正式发布。
