...
- [X] T007 [US1] 在 `scripts/audit-release.mjs` 实现 ZIP 中央目录预检、包内版本、白名单、禁用内容、双 helper 验收、SHA 文件、审计 JSON 和中文正文生成（依赖 T003、T006）
...
- [X] T017 依照 `specs/004-automatic-github-release/quickstart.md` 严格运行八项门禁和最终归档审计，确认构建与包内 helper、归档清单、SHA 和中文正文均通过
...
- 聚焦自动发布测试与八项门禁全部通过。
- 最终归档审计从 `.iinaplgz` 读取版本、清单、大小、SHA 和两份 helper 属性，并生成中文正文。
