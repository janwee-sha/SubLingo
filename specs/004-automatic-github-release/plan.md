...
新增 Node.js 发布元数据、最终归档审计和 GitHub Release 发布接口，并由 `.github/workflows/release.yml` 在远程 `main` 推送或限定的手动重试中编排。构建任务在 `macos-15` Arm64 runner 上固定 Node.js 24.18.0、npm 11 和校验后的 IINA v1.4.4，严格执行八项门禁；发布任务只消费经过审计的工作流产物，以 `contents: write` 创建或恢复 draft、核对资产后公开为 Latest。已发布版本只验证并跳过，任何对象冲突都失败且不覆盖。
...
- **存储**：只生成 `build/` 下的瞬时归档、校验、审计 JSON 和 Release 正文；不新增产品持久化。
- **测试**：Vitest 对元数据、ZIP 中央目录安全规则、native 属性失败和发布状态决策做回归；静态工作流契约测试固定触发器、权限、Action SHA 和门禁顺序；最后运行完整八项门禁。
...
- **约束**：稳定 SemVer；精确触发 SHA；官方 Action 固定完整 SHA；IINA v1.4.4 固定哈希；构建只读、发布可写；生产脚本不添加注释或非英语自然语言。
- **规模与范围**：一个 workflow、三个发布脚本、三组聚焦测试、004 SDD 与发布技能/验证约定更新。
...
**结构决策**：可本地执行的 Node.js 脚本拥有版本、ZIP 安全和发布状态机规则，workflow 只负责编排环境、严格门禁、权限和跨任务产物。最终归档审计直接读取 ZIP 中央目录，先拒绝危险条目再解包；发布接口通过 `gh` 读写 draft、tag 和资产，workflow 不复制状态机。

## 设计阶段

1. [research.md](./research.md) 固化 runner、IINA、Action SHA、权限、draft 与证据决策。
