...
- 刷新按钮具有稳定的可访问名称、键盘焦点、至少 32px 高度和 `aria-busy`；窄宽度下允许布局换为单列。
- 模型区域紧邻一个 `role="status" aria-live="polite"` 反馈槽。busy、成功与安全失败属于 `model-catalog` 区域，并参与现有全局唯一操作消息规则。
...
- 用户选择已知项时保存其精确值；选择 Custom 时保留或编辑自定义值。
