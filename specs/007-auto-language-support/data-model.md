...
- `draftTargetLanguageId`：WebView 当前选择，不进入播放逻辑。
- `dirty`：草稿是否偏离已提交值；为 true 时周期 poll 不覆盖草稿。
- `pendingRequestId`：保存中请求；完成前禁用重复提交。
- `operationState`：`idle | saving | saved | failed`。

关闭并重新打开 Sidebar 时草稿丢弃，重新以 Main committed state 初始化；完整退出 IINA 后 Main 从 Global preferences 恢复。

## 关系与所有权

```text
TargetLanguageOption catalog 1 -> 1 TargetLanguagePreference
Sidebar draft              1 -> 0..1 LanguageSaveAttempt
