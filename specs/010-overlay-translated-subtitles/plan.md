...
- **目标平台**：macOS 12+、IINA 1.4.0+；正式宿主验收覆盖 IINA 1.4.0 与当前发布版 1.4.4。
...
- **宿主边界**：IINA 的 `mpv.command` 仅暴露位置字符串参数并返回 `void`；自动化验证命令构造与应用状态，正式包负责证明 IINA 1.4.0/1.4.4 的真实渲染、清理、时序、裁切和宿主日志安全。
