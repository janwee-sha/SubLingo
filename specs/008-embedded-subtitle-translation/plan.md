...
在每个播放器 Main 内增加字幕源分类与准备协调器。外挂 SRT/ASS 继续使用现有读取路径；本地内嵌 `subrip`、`ass`/`ssa`、`mov_text` 轨通过随包、逐窗口隔离的 `sublingo-subtitle-extractor` 转成会话级 UTF-8 SRT，再进入现有 parser、有限前瞻、Provider、缓存和第二字幕发布流程。提取器静态链接裁剪后的 FFmpeg 8.1.2，只接受当前所选轨的精确容器流标识；远程媒体、图形字幕和无法证明身份的轨道失败关闭。
...
IINA selected primary subtitle
  -> Main: source classifier + SubtitlePreparationCoordinator
       ...
       embedded -> per-window authenticated extractor client
         -> sublingo-subtitle-extractor -> session SRT in @tmp
