...
- `007-auto-language-support` 已实现并通过其门禁。
- IINA 1.4.0 基线与当前发布固定版本；Apple Silicon 与 Intel 正式包环境。
...
随后执行最终归档审计。预期两个 native 组件均为 macOS 12 arm64/x86_64、可执行、签名有效且无非系统动态依赖；包内不存在源码、测试、构建缓存、运行时目录、路径、字幕或秘密材料；FFmpeg 对应源码资产与 lock/notice 一致。
...
4. 在同一文件的两条相同语言/title 字幕间切换，确认译文严格跟随实际所选轨。
5. 准备期间 seek，确认完成后从最新位置工作；随后换轨、换片、禁用和关闭，确认旧结果不创建 source 或第二字幕。
6. 在两个窗口同时使用不同媒体/轨道，确认状态、临时目录、缓存和译文不交叉。
7. 选择 PGS、VobSub、DVB、空/损坏文本轨和远程媒体，确认状态原因匹配、Provider 调用为 0、播放不受影响。
8. 制造超时，确认 15 秒后没有自动重试；点击 Retry 后只有新 attempt 可成功。
9. 切换回外挂 SRT/ASS，确认操作、有限前瞻、错误处理、隐私与第二字幕行为保持不变。
10. 正常关闭与强制结束 IINA 后重新启动，确认 `@tmp/sublingo-extraction` 无正文遗留；检查安全日志样本命中数为 0。

Apple Silicon 与 Intel、IINA 1.4.0 与当前固定发布版本均需完成。开发链接、`lipo` 输出或单一架构结果不能替代正式包宿主验收。

## 验收记录边界

只记录：包版本/hash、macOS/IINA/架构、样本类别与不透明编号、cue 数、准备耗时、状态、播放是否中断、第二字幕与清理结果。不得记录媒体文件名或路径、字幕正文、译文、Provider 请求、token、凭据或 native 原始错误。
